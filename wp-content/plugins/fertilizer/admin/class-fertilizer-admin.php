...
class Fertilizer_Admin {
	...
	public function __construct( $plugin_name, $version ) {
		...
		$this->fertilizer_options = get_option($this->plugin_name);
	}
	...
	public function enqueue_styles() {
		 ...
		 * defined in Fertilizer_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The Fertilizer_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class.
		 */

//		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/fertilizer-admin.css', array(), $this->version, 'all' );

		if ( 'settings_page_fertilizer' == get_current_screen() -> id ) {
			// CSS stylesheet for Color Picker
			wp_enqueue_style( 'wp-color-picker' );
			wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/fertilizer-admin.css', array( 'wp-color-picker' ), $this->version, 'all' );
		}
...
//		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/fertilizer-admin.js', array( 'jquery' ), $this->version, false );

		if ( 'settings_page_fertilizer' == get_current_screen() -> id ) {
			wp_enqueue_media();
			wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/fertilizer-admin.js', array( 'jquery', 'wp-color-picker' ), $this->version, false );
		}
	}
	...
	public function add_plugin_admin_menu()
	{
         ...
         *
         */
//		add_options_page('Fertilizers and Base Options Functions Setup', 'Fertilizers', 'manage_options', $this->plugin_name, array($this, 'display_plugin_setup_page'));
		add_menu_page('Fertilizers and Base Options Functions Setup', 'Fertilizers', 'manage_options', $this->plugin_name, array($this, 'display_plugin_setup_page'));
	}
	...
	{
		include_once('partials/fertilizer-admin-display.php');
	}
	 ...
	 * @since    1.0.0
	 */
	private function fertilizer_login_logo_css(){
		if(isset($this->fertilizer_options['login_logo_id']) && !empty($this->fertilizer_options['login_logo_id'])){
			$login_logo = wp_get_attachment_image_src($this->fertilizer_options['login_logo_id'], 'thumbnail');
	...
	private function fertilizer_login_background_color(){
		if(isset($this->fertilizer_options['login_background_color']) && !empty($this->fertilizer_options['login_background_color']) ){
			$background_color_css  = "body.login{ background:".$this->fertilizer_options['login_background_color']."!important;}";
			return $background_color_css;
		}
	}
	// Get Button and links color is set and different from #00A0D2 return it's css
	private function fertilizer_login_button_color(){
		if(isset($this->fertilizer_options['login_button_primary_color']) && !empty($this->fertilizer_options['login_button_primary_color']) ){
			$button_color = $this->fertilizer_options['login_button_primary_color'];
	...
	public function fertilizer_login_css(){
		if( !empty($this->fertilizer_options['login_logo_id']) || $this->fertilizer_login_background_color() != null || $this->fertilizer_login_button_color() != null){
			echo '<style>';
			if( !empty($this->fertilizer_options['login_logo_id'])){
				echo $this->fertilizer_login_logo_css();
			}
			if($this->fertilizer_login_background_color() != null){
				echo $this->fertilizer_login_background_color();
			}
			if($this->fertilizer_login_button_color() != null){
				echo $this->fertilizer_login_button_color();
			}
			echo '</style>';
		}
	}
